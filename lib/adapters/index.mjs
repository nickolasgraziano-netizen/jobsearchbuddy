...
async function workday(cfg) {
  ...
  // Workday pages 20 at a time. The real stop condition is posts.length < 20
  // below (Workday itself says "no more") - this offset ceiling is just a
  // generous safety backstop against a pathological/looping response, not a
  // meaningful cap in practice. It used to be 200, which was silently
  // truncating real results: Colorado Walmart alone currently has 411 live
  // postings (confirmed via a direct API call 2026-08-18), well past that
  // old ceiling, and every Workday source in this file (walmart-external-504,
  // walmart-people, sams-club, csu) was hitting exactly 200 every run - a
  // strong sign all of them were being cut off, not just Walmart. Keyless
  // and free, so there's no quota cost to paging further; the only cost is
  // ingest wall-clock time, which scales with each source's *real* count,
  ...
  for (let offset = 0; offset < 2000; offset += 20) {
    ...
    for (const p of posts) {
      // Confirmed live 2026-08-18: at least one real Workday posting
      // (Walmart req R-2542634) comes back with no title and no
      // externalPath at all - some kind of withdrawn/placeholder entry
      // that still shows up in search results. jobs.title and
      // jobs.apply_url are both NOT NULL, so a row like that crashes the
      // whole ingest's upsert (one bad record takes down all 16 sources'
      // worth of otherwise-good data in that run, not just this one).
      // Skip anything unusable at the source rather than pass it through.
...
async function jsearch(cfg, env) {
  ...
  // /search was retired for /search-v2 (confirmed 2026-08-18 via the live
  // RapidAPI playground, not the Overview docs text which still describes
  // the old one) - num_pages fetches multiple pages server-side in a single
  // request (1 credit/page) instead of a client-side page loop, and the
  // response wraps jobs in data.jobs rather than data being the array.
  const r = await req(
    `https://jsearch.p.rapidapi.com/search-v2?query=${encodeURIComponent(q)}&num_pages=${cfg.pages || 2}&country=us&date_posted=week`,
    { headers: { 'x-rapidapi-key': key, 'x-rapidapi-host': 'jsearch.p.rapidapi.com' } });
  if (!r.ok) throw new Error(`JSearch ${r.status}: ${r.json?.message || r.text?.slice(0, 200)}`);
  const data = r.json?.data?.jobs || [];
  const out = [];
  for (const j of data) {
    // job_salary_period is often HOUR for the retail/warehouse-type roles
    // this source is most useful for. Feeding an hourly rate through the
    // annual $Nk formatter (and the $40k+-style salary floor filter) as if
    // it were a yearly figure produces nonsense like "$0k-$0k" - only pass
    // salaryMin/Max through untouched when the period is actually annual
    // (or unstated, matching every other adapter's assumption); otherwise
    // render a plain "$17-19/hr" string and leave the filter-facing min/max
    // unset so an hourly job isn't wrongly hidden by a $40k+ pay filter.
    const period = (j.job_salary_period || '').toUpperCase();
    const unit = { HOUR: 'hr', DAY: 'day', WEEK: 'wk', MONTH: 'mo' }[period];
    const isAnnual = !unit;
    out.push(makeJob({
      ...
      salaryMin: isAnnual ? j.job_min_salary : undefined,
      salaryMax: isAnnual ? j.job_max_salary : undefined,
      salaryText: !isAnnual && (j.job_min_salary || j.job_max_salary)
        ? (j.job_min_salary && j.job_max_salary ? `$${j.job_min_salary}–${j.job_max_salary}/${unit}` : `$${j.job_min_salary || j.job_max_salary}/${unit}`)
        : undefined,
