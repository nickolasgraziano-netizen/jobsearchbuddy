...
// Reused across calls, and no-store'd - see lib/supabase.js's serverClient() for why.
let _admin;
...
export async function ingest({ log = console.log } = {}) {
  ...
  // Defense in depth against the class of bug found 2026-08-18: a single
  // Workday posting with no title reached here and its NOT NULL violation
  // crashed the *entire* upsert, discarding every other source's good data
  // for that run (not just the bad row) - the "Refresh now" button surfaced
  // it as a raw 500 to Nick. Fixed at the workday() adapter for that
  // specific case, but any future adapter could produce a row missing a
  // required field the same way, so drop (and report) anything unusable
  // right here too rather than let one bad row take the whole run down
  // again. jobs.title/apply_url are the only NOT NULL columns that come
  // from adapter data rather than being set by this file itself.
  const rows = [...merged.values()].filter(r => r.title && r.apply_url);
  const droppedInvalid = merged.size - rows.length;
